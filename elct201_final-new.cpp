 ...
 *
 *  IN THIS VERSION: clicking the pressure pad will lift door, wait one second, then lower door 
 * 
...
#define DOOR_WAIT_TIME 5.0f                 // Time in seconds that the door waits at the top
...
void iterate_and_check_timer() {
    ...
    if (timer_up_en) {
        ...
        if (timer_up_value <= 0) {
            ...
            light_LED(RED);
            timer_up_en = false;                    // disable up timer
            timer_up_value = DOOR_RISE_TIME;        // reset timer
            OutputMotorUp = 0;                      // Stop motor up
            timer_wait_en = true;                   // enable wait timer
        }
    }

    // Control timer for motor waiting at top
    if (timer_wait_en) {
        // Advance by CYCLE_TIME each cycle if wait timer is enabled
        timer_wait_value -= CYCLE_TIME;

        if (timer_wait_value <= 0) {
            // If timer reaches zero, start motor down and reset timer
            cout << "Starting Motor Down" << endl;
            light_LED(GREEN);
            timer_wait_en = false;                  // disable wait timer
            timer_wait_value = DOOR_RISE_TIME;      // reset timer
            OutputMotorDown = 1;                    // Start motor down
            timer_down_en = true;                   // enable down timer
        }
    }
    ...
    // Control timer for motor going down
        ...
        if (timer_down_value <= 0) {
            ...
            light_LED(RED);
            timer_down_en = false;                  // disable down timer
            timer_down_value = DOOR_FALL_TIME;      // reset timer
            OutputMotorDown = 0;                    // Stop motor down
            button_en = true;                       // Enable pressure pad now that motor is back down
        }
    }
}
...
void pressure_detected() {
    ...
    // Start motor up and down process
    if (button_en){
